...
| 코어 게임 루프 | 95% | 이동, 스폰, 공격, 드롭, 마그넷, 맵 아이템, 엘리트, 보스 완료. 보물상자 미구현 |
| 무기 시스템 | 100% | 8/8종 구현, 레벨업 시스템 완료 |
| 적 시스템 | 92% | 7종 일반 적 + 3종 엘리트 + 보스 1종. 뿌리 손/2번째 보스 미구현 |
| 패시브 시스템 | 100% | 시스템 완료, 10/10종 패시브 구현 |
| 레벨업 UI | 90% | 카드 3장 선택, 무기/패시브 업그레이드 동작. 비주얼 Polish 필요 |
| 메타 진행 | 0% | 기억 시스템 미구현 |
| 스토리 | 0% | 단서/서재 미구현 |
...
| SpawnManager | **완료** | 시간별 적 스폰, 난이도 곡선(1.5s→0.4s), 화면밖 디스폰, 킬 카운트, 엘리트 웨이브(5분마다), 보스 스폰(10분) |
...
### 11. 씬 구성 (scenes/)
- **main.tscn/gd**: 진입점, ESC 일시정지 토글, MENU→자동 재시작
- **stage.tscn/gd**: 플레이어 배치, 매니저 등록, UI 셋업, 디버그 격자
...
- [ ] 기억 조각 (골드) 획득 시스템
- [ ] 기억의 제단 (영구 업그레이드 8종)
- [ ] 세이브/로드 (`user://meta_progress.tres`)
